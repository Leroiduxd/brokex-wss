...
// --- helper pour respecter 10 req/s ---
const MIN_GAP_MS = 100; // 10 appels / seconde
...
async function fetchAllPricesAndBroadcast() {
  if (isFetching) return; // évite un overlap si la boucle prend > 1000ms
  isFetching = true;
  ...
  try {
    ...
    for (const pair of PAIRS) {
      try {
        ...
        responses.push({ pair, data });
      } catch {
        responses.push({ pair, data: null });
      }
      // Espacement pour respecter 10 req/s
